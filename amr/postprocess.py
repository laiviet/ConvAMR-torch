...
if args.fairseq:
  ...
  # Post process
  print('Save AMR file: {}'.format(system_amr_path))
  ...
  if args.sentenceFile:
    ...
    with codecs.open(args.sentenceFile, 'r',encoding='utf-8') as f:
    ...
    if len(sentences) == len(stripped_data):
      ...
      for idx, (snt, amr_text) in enumerate(zip(sentences, result)):
        text_lines.append(u'# ::id %d\n# ::snt %s\n%s\n'%(idx, snt.strip(), amr_text))

      system_output_path = system_tmp_path + '.amr.txt'
      print('Save file at: ' + system_output_path)
      with codecs.open(system_output_path, 'w',encoding='utf-8') as f:
        f.write('\n'.join(text_lines))
    else:
